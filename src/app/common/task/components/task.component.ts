...
import { MainTaskContainer } from '../services/taskbag-container.service';
...
export class TaskComponent implements OnChanges, OnDestroy, OnInit {
    @Input() task: Task;
    @Input()
    _dropLocation: any;
    ...
    constructor(public treeService: TreeService, private mianTaskContainer: MainTaskContainer, private biz: TaskBiz, private container: TaskBagContainer, private posCalculationRule: PosCalculationRule) {

    }
    ngOnDestroy() {
        if (this.mianTaskContainer._isFirstInit) {
            this.mianTaskContainer._isFirstInit = false;
        }
    }
    ngOnInit() {
        if (this.mianTaskContainer._isFirstInit) {
    ...
    onDragStart($event) {
        ...
        // first 
        setTimeout(() => {
            //mianTaskContainer._dragTask = this.task;
            this.mianTaskContainer._dragTaskComponent = this;
    ...
    onDragEnd() {
        ...
        this.mianTaskContainer._dragTaskComponent.task.is_hidden = false;
    ...
    onDrop($event) {
        $event.preventDefault();
        let dragComponent = this.mianTaskContainer._dragTaskComponent;
