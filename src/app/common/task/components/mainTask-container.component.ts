...
@Component({
    moduleId: module.id,
    selector: 'mianTaskContainer',
    providers: [TreeService,TaskBiz,TaskDropSlotBiz,TaskBagBiz,PosCalculationRule,MainTaskContainer],
    styleUrls:['../templates/res/css/task/tree.css'],
    templateUrl: `
    <!-- -->
    <taskbag *ngFor = "let taskbag of trees"
    [taskbag]="taskbag"
    >
    </taskbag>
    `,
})
export class MianTaskContainerComponent implements OnChanges {

    @Input()  trees:TaskBag[];
    
    constructor(public treeService:TreeService,private mianTaskContainer:MainTaskContainer) {
     
    }
    ngOnChanges(changes) {
        console.log('tree container');
        this.mianTaskContainer._isFirstInit = true;
